...
#include <stdlib.h>
/*引脚配置*/
#define OLED_W_SCL(x)		digitalWrite(3,x)
#define OLED_W_SDA(x)		digitalWrite(4,x)
...
{
	OLED_Init();
	unsigned char line = atoi(argv[1]);
	unsigned char column = atoi(argv[2]);
	OLED_ShowString(line,column,argv[3]);
}